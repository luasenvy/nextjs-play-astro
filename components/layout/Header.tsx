...
const menuitems = [
  ...
  },
  {
    ...
    children: [
      ...
      { title: "Pricing Page", path: "/pricing" },
...
export default function Header() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => setIsSticky(window.scrollY > header.offsetTop);
  ...
  return (
    <header
      ...
      className={classnames(
        ...
        {
          "sticky-header absolut bg-transparen z-40": isSticky,
        }
      )}
    >
          ...
          <div className="w-60 max-w-full px-4">
            <Link href="/" className="navbar-logo block w-full py-5">
              {isHome ? (
                <Image
                  ref={headerLogoRef}
                  src={isSticky ? (isDark ? LogoWhiteImage : LogoImage) : LogoWhiteImage}
                  alt="logo"
                  className="header-logo w-full"
                />
              ) : (
                <>
                  <Image src={LogoImage} alt="logo" className="w-full dark:hidden" />
                  <Image src={LogoWhiteImage} alt="logo" className="w-full hidden dark:block" />
                </>
              )}
            </Link>
          </div>
          <div className="flex w-full items-center justify-between px-4">
            <div>
              ...
              <nav
                id="navbarCollapse"
                className={classnames(
                  "absolute right-4 top-full w-full max-w-[250px] rounded-lg bg-white dark:bg-dark-2 py-5 shadow-lg lg:static lg:block lg:w-full lg:max-w-full lg:bg-transparent dark:lg:bg-transparent lg:py-0 xl:px-4 lg:shadow-none 2xl:px-6",
                  {
                    navbarTogglerActive: open,
                    hidden: !open,
                  }
                )}
              >
                <ul className="block lg:flex 2xl:ml-20">
                  {menuitems.map((item, index) => {
                    if (!item.children) {
                      return (
                        <li key={`item-${index}`} className="group relative">
                          <Link
                            href={item.path}
                            onClick={() => setOpen(false)}
                            className={classnames(String(index), {
                              "ud-menu-scroll mx-8 flex py-2 font-medium text-base text-dark dark:text-white group-hover:text-primary lg:mr-0 lg:ml-7 lg:inline-flex lg:py-6 lg:px-0 lg:text-white lg:group-hover:text-white lg:group-hover:opacity-70 xl:ml-10":
                                isHome,
                              "ud-menu-scroll mx-8 flex py-2 font-medium text-base text-dark dark:text-white group-hover:text-primary lg:mr-0 lg:ml-7 lg:inline-flex lg:py-6 lg:px-0 lg:text-body-color dark:lg:text-dark-6 xl:ml-10":
                                !isHome,
                            })}
                          >
                            {item.title}
                    ...
                    if (item.children) {
                      return (
                        <li key={`item-${index}`} className="submenu-item group relative">
                          <Link
                            href={item.path}
                            onClick={() => setSubmenuOpen((prev) => !prev)}
                            className={classnames(String(index), {
                              "relative mx-8 flex items-center justify-between py-2 font-medium text-base text-dark dark:text-white group-hover:text-primary lg:mr-0 lg:ml-8 lg:inline-flex lg:py-6 lg:pl-0 lg:pr-4 lg:text-white lg:group-hover:text-white lg:group-hover:opacity-70 xl:ml-10":
                                isHome,
                              "relative mx-8 flex items-center justify-between py-2 font-medium text-base text-primary group-hover:text-primary lg:mr-0 lg:ml-8 lg:inline-flex lg:py-6 lg:pl-0 lg:pr-4 xl:ml-10":
                                !isHome,
                            })}
                          >
                            {item.title}
                          ...
                          <div
                            className={classnames(
                              "submenu relative top-full left-0 w-[250px] rounded-sm bg-white dark:bg-dark-2 p-4 transition-[top] duration-300 group-hover:opacity-100 lg:invisible lg:absolute lg:top-[110%] lg:block lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full",
                              { hidden: !submenuOpen }
                            )}
                          >
                            {item.children.map((item, i) => (
                              <Link
                                ...
                                href={item.path}
                                onClick={() => setOpen(false)}
                                className="block rounded py-[10px] px-4 text-sm text-body-color dark:text-dark-6 hover:text-primary dark:hover:text-primary"
                              >
                                {item.title}
                              </Link>
                            ))}
            ...
            <div className="justify-end pr-16 flex items-center lg:pr-0">
              ...
              >
                <span
                  className={classnames("block dark:hidden", {
                    "text-white": isHome,
                    "text-dark dark:text-white": !isHome,
                  })}
                >
                  <svg
              ...
              <div className="hidden sm:flex">
                <Link
                  href="/signin"
                  className={classnames({
                    "loginBtn py-2 px-[22px] text-base font-medium text-white hover:opacity-70":
                      isHome,
                    "loginBtn py-2 px-[22px] text-base font-medium text-dark dark:text-white hover:opacity-70":
                      !isHome,
                  })}
                >
                  Sign In
                ...
                <Link
                  href="/signup"
                  className={classnames({
                    "signUpBtn rounded-md bg-white bg-opacity-20 py-2 px-6 text-base font-medium text-white duration-300 ease-in-out hover:bg-opacity-100 hover:text-dark":
                      isHome,
                    "signUpBtn rounded-md bg-primary py-2 px-6 text-base font-medium text-white duration-300 ease-in-out hover:bg-blue-dark":
                      !isHome,
                  })}
                >
                  Sign Up
